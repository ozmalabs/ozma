...
use std::sync::{Arc, Mutex};
use serde_json::Value;
use super::types::{ButtonStyle, LightStyle, Color, Invert};

/// Base trait for MIDI controls
pub trait MidiControl: Send + Sync {
    /// Process incoming MIDI message, return state delta
    fn on_midi_message(&mut self, message: &[u8]) -> Option<HashMap<String, Value>>;
...
impl MidiFader {
    pub fn new(name: String, cc: u8, touch_note: Option<u8>, midi_out: Arc<Mutex<Option<midir::MidiOutputConnection>>>) -> Self {
        Self {
            name,
            value: Value::Number(serde_json::Number::from(0)),
            lockout: false,
...
impl MidiControl for MidiFader {
    fn on_midi_message(&mut self, message: &[u8]) -> Option<HashMap<String, Value>> {
        if message.len() >= 3 && message[0] == 0xB0 && message[1] == self.cc {
            // Control change message
            self.value = Value::Number(serde_json::Number::from(message[2]));
            let mut delta = HashMap::new();
            delta.insert("value".to_string(), self.value.clone());
            Some(delta)
        } else if let Some(touch_note) = self.touch_note {
...
impl MidiButton {
    ...
    fn update_light(&self, midi_out: Arc<Mutex<Option<midir::MidiOutputConnection>>>) {
        let on = match self.light_style {
            LightStyle::False => false,
            LightStyle::AlwaysOn => true,
            LightStyle::Momentary => self.pressed,
...
impl MidiRotary {
    pub fn new(name: String, cc: u8, midi_out: Arc<Mutex<Option<midir::MidiOutputConnection>>>) -> Self {
        Self {
            name,
            value: Value::Number(serde_json::Number::from(0)),
            lockout: false,
