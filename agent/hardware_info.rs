...
pub struct CacheInfo {
    pub level: u8,          // 1, 2, 3
    pub size_kb: u32,
    pub type_: String,      // Data, Instruction, Unified
    pub ways: u32,
    pub sets: u32,
}
...
pub struct MemorySlotInfo {
    pub slot: String,
    pub size_gb: f32,
    pub type_: String,          // DDR4, DDR5, LPDDR5, etc.
    pub speed_mhz: u32,
    pub configured_speed_mhz: u32,
    pub manufacturer: String,
    pub part_number: String,
    pub serial_number: String,
    pub form_factor: String,   // DIMM, SO-DIMM, etc.
    pub rank: String,
    pub bank: String,
    pub channel: String,
    pub timing: MemoryTiming,
...
pub struct GpuInfo {
    ...
    pub vram_gb: f32,
    pub pcie_slot: i32,
    pub pcie_width: u32,     // x4, x8, x16
    pub pcie_gen: u32,       // 3, 4, 5
    pub driver_version: String,
    pub bios_version: String,
    pub cuda_cores: u32,
    pub shader_processors: u32,
...
pub struct SmartAttribute {
    pub id: u32,
    pub name: String,
    pub value: u32,          // normalized 0-200
    pub worst: u32,
    pub raw: u64,
    pub threshold: u32,
    pub flags: u32,
...
pub struct NvmeData {
    ...
    pub available_spare_percent: u32,
    pub available_spare_threshold_percent: u32,
    pub percentage_used: u32,         // wear level (0 = new, 100 = worn)
    pub data_units_written: u64,      // in 512KiB units
    pub data_units_read: u64,
    pub host_write_commands: u64,
    pub host_read_commands: u64,
    pub media_errors: u64,
    ...
    pub power_on_hours: u64,
    pub unsafe_shutdowns: u64,
    pub critical_warning: u32,        // bitmask; 0 = healthy
}
...
pub struct SmartData {
    pub health_percent: u32,
    pub overall_status: String,   // PASSED, FAILED, OLD_AGE, PRE-FAIL
    pub power_on_hours: u64,
    pub start_stop_count: u64,
    pub reallocated_sectors: u64,
    pub pending_sectors: u64,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct StorageInfo {
    pub device: String,        // /dev/sda, /dev/nvme0n1, \\.\PhysicalDrive0
    pub model: String,
    pub serial: String,
    pub firmware: String,
    pub interface: String,     // NVMe, SATA, SAS, USB
    pub form_factor: String,   // 2.5", 3.5", M.2, U.2
    pub capacity_bytes: u64,
    pub rotational: bool,
    pub rpm: u32,
    // Live sensors
...
pub struct BatteryInfo {
    ...
    pub capacity_design_mwh: u64,
    pub capacity_full_mwh: u64,
    pub capacity_now_mwh: u64,
    pub charge_rate_mw: u64,
    pub voltage_mv: u64,
    pub temperature_c: f32,
    pub cycle_count: u64,
    pub technology: String,    // Li-ion, LiPo, etc.
}

impl BatteryInfo {
    pub fn health_percent(&self) -> u32 {
        if self.capacity_design_mwh > 0 {
            std::cmp::min(100, (self.capacity_full_mwh * 100 / self.capacity_design_mwh) as u32)
        } else {
            0
        }
    }
    ...
    pub fn charge_percent(&self) -> f32 {
        if self.capacity_full_mwh > 0 {
            std::cmp::min(100.0, self.capacity_now_mwh as f32 * 100.0 / self.capacity_full_mwh as f32)
        } else {
            0.0
        }
    }
...
impl LinuxHardwareCollector {
    ...
    pub fn collect(&mut self) -> Result<HardwareSnapshot> {
        let mut snap = HardwareSnapshot::default();
        snap.timestamp = self.get_timestamp();
        snap.hostname = self.get_hostname()?;
        snap.os = "Linux".to_string();
        snap.os_version = self.get_os_version()?;

        snap.cpu = self.collect_cpu()?;
        snap.memory_slots = self.collect_memory()?;
        snap.total_memory_gb = snap.memory_slots.iter().map(|s| s.size_gb).sum::<f32>().max(self.total_ram_gb()?);
        snap.gpus = self.collect_gpus()?;
        snap.storage = self.collect_storage()?;
        snap.motherboard = self.collect_motherboard()?;
    ...
    fn get_os_version(&self) -> Result<String> {
        let content = self.read_file_to_string("/proc/version")?;
    ...
    fn collect_cpu(&self) -> Result<CpuPackageInfo> {
        ...
        for line in cpuinfo.lines() {
            ...
            match key {
                "model name" if cpu.brand.is_empty() => {
                    cpu.brand = value.to_string();
                    cpu.model = value.to_string();
                }
                "vendor_id" if cpu.socket.is_empty() => {
                    cpu.socket = if value.contains("AMD") {
                        "AM5".to_string()
                    } else if value.contains("Intel") {
                        "LGA".to_string()
                ...
                "cpu MHz" if cpu.base_clock_mhz == 0.0 => {
                    if let Ok(freq) = value.parse::<f32>() {
                        cpu.base_clock_mhz = freq;
                    }
                }
                "flags" if cpu.instructions.is_empty() => {
                    let known = [
                        "avx", "avx2", "avx512f", "avx512bw", "avx512cd",
                        "avx512dq", "avx512vl", "avx512vnni", "avx512bf16",
                        "amx_bf16", "amx_tile", "sse4_2", "sse4_1", "sse4a",
        ...
        // Count physical IDs and cores
        let physical_ids: std::collections::HashSet<String> = 
            regex::Regex::new(r"physical id\s*:\s*(\d+)")?
                .captures_iter(&cpuinfo)
                .map(|cap| cap[1].to_string())
                .collect();
        cpu.sockets = std::cmp::max(1, physical_ids.len());
        
        if let Some(coref) = regex::Regex::new(r"cpu cores\s*:\s*(\d+)")?.captures(&cpuinfo) {
            if let Ok(cores) = coref[1].parse::<usize>() {
                cpu.physical_cores = cores * cpu.sockets;
            }
        }
            ...
            if output.status.success() {
                if let Ok(json_str) = String::from_utf8(output.stdout) {
                    if let Ok(data) = serde_json::from_str::<serde_json::Value>(&json_str) {
                        if let Some(lscpu_array) = data["lscpu"].as_array() {
                            let mut entries = HashMap::new();
                            for entry in lscpu_array {
                                if let (Some(field), Some(data_val)) = 
                                    (entry["field"].as_str(), entry["data"].as_str()) {
                                    entries.insert(field.trim_end_matches(':').to_string(), data_val.to_string());
                                }
                            }
                            
                            if cpu.brand.is_empty() {
                                if let Some(model) = entries.get("Model name") {
                                    cpu.brand = model.clone();
                                    cpu.model = model.clone();
                                }
                            }
                            
                            // Parse cache info
                            for (key, level) in &[("L1d cache", 1), ("L1i cache", 1), ("L2 cache", 2), ("L3 cache", 3)] {
                                if let Some(val) = entries.get(*key) {
                                    if let Some(captures) = regex::Regex::new(r"(\d+(?:\.\d+)?)\s*(K|M|G)")?.captures(val) {
                                        if let Ok(size) = captures[1].parse::<f32>() {
                                            ...
                                            let cache_type = if key.contains("d cache") {
                                                "Data"
                                            } else if key.contains("i cache") {
                                                "Instruction"
                                            } else {
                                                "Unified"
                                            };
                                            
                                            cpu.caches.push(CacheInfo {
                                                level: *level,
                                                size_kb,
                                                type_: cache_type.to_string(),
                                                ways: 0,
                                                sets: 0,
                                            });
                                        }
                            ...
                            // Max MHz
                            if let Some(max_mhz) = entries.get("CPU max MHz") {
                                if let Ok(freq) = max_mhz.parse::<f32>() {
                                    cpu.boost_clock_mhz = freq;
                                }
                            }
                            
                            if cpu.base_clock_mhz == 0.0 {
                                if let Some(min_mhz) = entries.get("CPU min MHz") {
                                    if let Ok(freq) = min_mhz.parse::<f32>() {
                                        cpu.base_clock_mhz = freq;
                                    }
                                }
                            }
        ...
        if let Ok(pkg_temp) = self.read_hwmon_package_temp() {
            cpu.package_temp_c = pkg_temp;
        } else if !cpu.cores.is_empty() {
            cpu.package_temp_c = cpu.cores.iter()
                .map(|c| c.temperature_c)
        ...
        // RAPL power
        let (pkg, core, uncore, dram) = self.read_rapl_power()?;
        cpu.package_power_w = pkg;
        cpu.core_power_w = core;
        cpu.uncore_power_w = uncore;
        cpu.dram_power_w = dram;
    ...
    fn collect_cpu_cores(&self, num_cores: usize) -> Result<Vec<CpuCoreInfo>> {
        let mut cores = Vec::new();
        let hwmon_temps = self.read_hwmon_core_temps()?;
        ...
        for i in 0..num_cores {
            ...
            let freq_path = format!("/sys/devices/system/cpu/cpu{}/cpufreq/scaling_cur_freq", i);
            if Path::new(&freq_path).exists() {
                if let Ok(content) = self.read_file_to_string(&freq_path) {
    ...
    fn read_hwmon_core_temps(&self) -> Result<Vec<f32>> {
        ...
        for entry in fs::read_dir(hwmon_base)? {
            ...
            let name = self.read_file_to_string(&name_path.to_string_lossy())?
                .trim()
                .to_string();
            
            if !["coretemp", "k10temp", "zenpower", "nct6775", "nct6779", "w83795g"].contains(&name.as_str()) {
                continue;
            }
            ...
            for entry in fs::read_dir(&hwmon_dir)? {
                ...
                let label_path = file.with_file_name(file_name.replace("input", "label"));
                let label = if label_path.exists() {
                    self.read_file_to_string(&label_path.to_string_lossy())?
                        .trim()
                        .to_string()
                } else {
                    String::new()
                ...
                if let Some(captures) = regex::Regex::new(r"Core\s+(\d+)")?.captures(&label) {
                    if let Ok(core_idx) = captures[1].parse::<usize>() {
                        if let Ok(temp_str) = self.read_file_to_string(&file.to_string_lossy()) {
                            if let Ok(temp) = temp_str.trim().parse::<f32>() {
                                temps.insert(core_idx, temp / 1000.0);
                            }
                        }
        ...
        let max_idx = *temps.keys().max().unwrap_or(&0);
        Ok((0..=max_idx).map(|i| *temps.get(&i).unwrap_or(&0.0)).collect())
    }
    ...
    fn read_hwmon_package_temp(&self) -> Result<f32> {
        ...
        for entry in fs::read_dir(hwmon_base)? {
            ...
            for entry in fs::read_dir(&hwmon_dir)? {
                ...
                if label.contains("Package") || label.contains("Tdie") || label.contains("Tccd") {
                    let temp_str = self.read_file_to_string(&file.to_string_lossy())?;
                    return Ok(temp_str.trim().parse::<f32>()? / 1000.0);
                }
            }
        }
    ...
    fn read_rapl_power(&self) -> Result<(f32, f32, f32, f32)> {
        ...
        for entry in fs::read_dir(rapl_base)? {
            let zone = entry?.path();
            if !zone.is_dir() {
                continue;
            }
            
            let name_path = zone.join("name");
            ...
            // Sample energy over 100ms for instantaneous power
            let e1 = self.read_file_to_int(&energy_path.to_string_lossy())?;
            std::thread::sleep(std::time::Duration::from_millis(100));
            let e2 = self.read_file_to_int(&energy_path.to_string_lossy())?;
            
            let power = (e2 - e1) as f32 / 1e5; // uj/0.1s → watts
    ...
    fn read_vcore(&self) -> Result<f32> {
        ...
        for entry in fs::read_dir(hwmon_base)? {
            ...
            let valid_names = [
                "nct6775", "nct6779", "nct6776", "nct6791", "nct6792",
                "nct6793", "nct6795", "nct6796", "nct6797", "nct6798",
                "it8720f", "it8728f", "it8771e", "w83627ehf", "asus_ec"
            ];
            ...
            for entry in fs::read_dir(&hwmon_dir)? {
                ...
                if label.contains("vcore") || label.contains("cpu") {
                    let voltage_str = self.read_file_to_string(&file.to_string_lossy())?;
                    return Ok(voltage_str.trim().parse::<f32>()? / 1000.0);
                }
            }
        }
    ...
    fn collect_memory(&self) -> Result<Vec<MemorySlotInfo>> {
        ...
        for block in output_str.split("\n\n") {
            ...
            if block.contains("No Module Installed") {
                continue;
            }
            
            let mut slot = MemorySlotInfo::default();
            ...
            for line in block.lines() {
                ...
                match key {
                    ...
                    "Bank Locator" => slot.bank = value.to_string(),
                    "Size" => {
                        if let Some(captures) = regex::Regex::new(r"(\d+(?:\.\d+)?)\s*(MB|GB|TB)")?.captures(value) {
                            if let Ok(size) = captures[1].parse::<f32>() {
                                let unit = &captures[2].to_uppercase();
                                slot.size_gb = match unit.as_str() {
                                    "MB" => size / 1024.0,
                                    "GB" => size,
                                    "TB" => size * 1024.0,
                                    _ => size,
                    ...
                    "Type" => slot.type_ = value.to_string(),
                    "Speed" => {
                        if let Some(captures) = regex::Regex::new(r"(\d+)")?.captures(value) {
                            if let Ok(speed) = captures[1].parse::<u32>() {
                                slot.speed_mhz = speed;
                            }
                        }
                    }
                    ...
                    "Configured Voltage" => {
                        if let Some(captures) = regex::Regex::new(r"([\d.]+)")?.captures(value) {
                            if let Ok(voltage) = captures[1].parse::<f32>() {
                                slot.timing.voltage_v = voltage;
                            }
                        }
                    }
                    "CAS Latency" => {
                        if let Some(captures) = regex::Regex::new(r"(\d+)")?.captures(value) {
                            if let Ok(cl) = captures[1].parse::<u32>() {
                                slot.timing.cl = cl;
                            }
                        }
                    }
        ...
        // Memory temperature from hwmon jc42
        let hwmon_base = "/sys/class/hwmon";
        if Path::new(hwmon_base).exists() {
            let mut dimm_temps = Vec::new();
            ...
            for entry in fs::read_dir(hwmon_base)? {
                ...
                if name == "jc42" {
                    ...
                    if temp_path.exists() {
                        let temp_str = self.read_file_to_string(&temp_path.to_string_lossy())?;
                        if let Ok(temp) = temp_str.trim().parse::<f32>() {
                            dimm_temps.push(temp / 1000.0);
                        }
                    }
    ...
    fn total_ram_gb(&self) -> Result<f32> {
        let meminfo = self.read_file_to_string("/proc/meminfo")?;
        for line in meminfo.lines() {
            if line.starts_with("MemTotal:") {
                let parts: Vec<&str> = line.split_whitespace().collect();
                if parts.len() > 1 {
                    if let Ok(kb) = parts[1].parse::<u64>() {
                        return Ok(kb as f32 / (1024.0 * 1024.0));
                    }
                }
            }
        }
    ...
    fn collect_gpus(&self) -> Result<Vec<GpuInfo>> {
            ...
            if output.status.success() {
                ...
                for line in output_str.lines() {
                    ...
                    let gpu = GpuInfo {
                        ...
                        vram_gb: parse_f32(parts[2]) / 1024.0,
                        vram_used_gb: parse_f32(parts[3]) / 1024.0,
                        driver_version: parts[4].to_string(),
                        temperature_c: parse_f32(parts[5]),
                        vram_temp_c: parse_f32(parts[6]),
                        core_clock_mhz: parse_f32(parts[7]),
                        memory_clock_mhz: parse_f32(parts[8]),
                        shader_clock_mhz: parse_f32(parts[9]),
                        power_w: parse_f32(parts[10]),
                        power_limit_w: parse_f32(parts[11]),
                        fan_percent: parse_f32(parts[12]),
                        utilization_percent: parse_f32(parts[13]),
                        nvenc_usage_percent: parse_f32(parts[14]),
                        nvdec_usage_percent: parse_f32(parts[15]),
                        pcie_width: parts[16].parse().unwrap_or(0),
                        pcie_gen: parts[17].parse().unwrap_or(0),
                        bios_version: parts.get(18).unwrap_or(&"").to_string(),
                        ..Default::default()
                    };
        ...
        // AMD via hwmon if no NVIDIA found
        if gpus.is_empty() {
            gpus.extend(self.collect_amd_gpus()?);
        }
    ...
    fn collect_amd_gpus(&self) -> Result<Vec<GpuInfo>> {
        ...
        for entry in fs::read_dir(hwmon_base)? {
            ...
            for fname in &["product_name", "label"] {
                let p = device_path.join(fname);
                if p.exists() {
                    gpu.model = self.read_file_to_string(&p.to_string_lossy())?
                        .trim()
                        .to_string();
                    break;
                }
            ...
            for entry in fs::read_dir(&hwmon_dir)? {
                ...
                let val_str = self.read_file_to_string(&file.to_string_lossy())?;
            ...
            for entry in fs::read_dir(&hwmon_dir)? {
                ...
                if !file_name.starts_with("power") || !file_name.ends_with("_average") {
                    continue;
                }
                
                let power_str = self.read_file_to_string(&file.to_string_lossy())?;
                if let Ok(power) = power_str.trim().parse::<f32>() {
                    gpu.power_w = power / 1e6;
                    break;
                }
            }
            ...
            if fan_path.exists() {
                let fan_str = self.read_file_to_string(&fan_path.to_string_lossy())?;
            ...
            // GPU clock via pp_dpm_sclk
            let sclk = device_path.join("pp_dpm_sclk");
            if sclk.exists() {
                let sclk_content = self.read_file_to_string(&sclk.to_string_lossy())?;
                for line in sclk_content.lines() {
                    if line.contains('*') {
                        if let Some(captures) = regex::Regex::new(r"(\d+)Mhz")?.captures(line) {
                            if let Ok(freq) = captures[1].parse::<f32>() {
                                gpu.core_clock_mhz = freq;
                                break;
                            }
                        }
                    }
            ...
            // VRAM
            let vram_total = device_path.join("mem_info_vram_total");
            let vram_used = device_path.join("mem_info_vram_used");
            
            if vram_total.exists() {
                let total_str = self.read_file_to_string(&vram_total.to_string_lossy())?;
                if let Ok(total) = total_str.trim().parse::<f32>() {
                    gpu.vram_gb = total / 1e9;
                }
            }
            
            if vram_used.exists() {
                let used_str = self.read_file_to_string(&vram_used.to_string_lossy())?;
                if let Ok(used) = used_str.trim().parse::<f32>() {
                    gpu.vram_used_gb = used / 1e9;
                }
            }
    ...
    fn collect_storage(&mut self) -> Result<Vec<StorageInfo>> {
        let mut devices = Vec::new();
        let block = "/sys/block";
        
        if !Path::new(block).exists() {
            return Ok(devices);
        }
        ...
        for entry in fs::read_dir(block)? {
            let dev_path = entry?.path();
            let name = dev_path.file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("")
                .to_string();
            
            if regex::Regex::new(r"^(loop|ram|dm-|sr|fd|zram)")?.is_match(&name) {
                continue;
            }
            
            let size_path = dev_path.join("size");
            let size_sectors = if size_path.exists() {
                self.read_file_to_int(&size_path.to_string_lossy())?
            } else {
                0
            };
            ...
            // Model, serial, rotational, interface
            let dpath = dev_path.join("device");
            for (fname, attr) in &[("model", "model"), ("serial", "serial"), ("vendor", ""), ("firmware_rev", "firmware")] {
                let p = dpath.join(fname);
                if p.exists() {
                    let val = self.read_file_to_string(&p.to_string_lossy())?
                        .trim()
                        .to_string();
            ...
            let rotational_path = dev_path.join("queue").join("rotational");
            if rotational_path.exists() {
                let rotational_str = self.read_file_to_string(&rotational_path.to_string_lossy())?;
                dev.rotational = rotational_str.trim().parse::<u32>()? != 0;
            }
            ...
            if name.starts_with("nvme") {
                ...
                let transport_path = dpath.join("transport");
                if transport_path.exists() {
                    dev.interface = self.read_file_to_string(&transport_path.to_string_lossy())?
                        .trim()
                        .to_uppercase();
                } else {
                    dev.interface = if dev.rotational { "HDD/SATA" } else { "SATA" }.to_string();
                }
            }
            ...
            let stat_path = dev_path.join("stat");
            if stat_path.exists() {
                let stat_content = self.read_file_to_string(&stat_path.to_string_lossy())?;
                let stat_parts: Vec<&str> = stat_content.split_whitespace().collect();
                
                if stat_parts.len() >= 7 {
                    if let (Ok(r_sectors), Ok(w_sectors)) = 
                        (stat_parts[2].parse::<u64>(), stat_parts[6].parse::<u64>()) {
                        
                        let prev = self.prev_disk.get(&name);
                        if let Some((pr, pw, pt)) = prev {
                            let dt = now - pt;
                            if dt > 0.0 {
                                dev.read_rate_mbs = (r_sectors - pr) as f32 * 512.0 / 1e6 / dt as f32;
                                dev.write_rate_mbs = (w_sectors - pw) as f32 * 512.0 / 1e6 / dt as f32;
                            }
                        }
                        self.prev_disk.insert(name.clone(), (r_sectors, w_sectors, now));
                    }
    ...
    fn collect_smart(&self, device: &str, interface: &str) -> Result<Option<SmartData>> {
        ...
        // bit 0 = SMART pre-fail warning, that's OK
        if !output.status.success() && (output.status.code().unwrap_or(1) & !0x01) != 0 {
            return Ok(None);
        }
        ...
        let data: serde_json::Value = serde_json::from_str(&json_str)
            .context("Failed to parse smartctl JSON")?;
        
        let mut smart = SmartData::default();
        
        // Overall health
        if let Some(status) = data["smart_status"].as_object() {
            smart.overall_status = if status["passed"].as_bool().unwrap_or(false) {
                "PASSED".to_string()
            } else {
                "FAILED".to_string()
            };
        ...
        // Power-on hours
        if let Some(poh) = data["power_on_time"].as_object() {
            smart.power_on_hours = poh["hours"].as_u64().unwrap_or(0);
        }
        
        // Temperature
        if let Some(temp) = data["temperature"].as_object() {
            smart.temperature_c = temp["current"].as_f64().unwrap_or(0.0) as f32;
        }
        
        // ATA attributes
        if let Some(ata_attrs) = data["ata_smart_attributes"].as_object()
            .and_then(|o| o["table"].as_array()) {
            for attr in ata_attrs {
                let id = attr["id"].as_u64().unwrap_or(0) as u32;
                let name = attr["name"].as_str().unwrap_or("").to_string();
                let value = attr["value"].as_u64().unwrap_or(0) as u32;
                let worst = attr["worst"].as_u64().unwrap_or(0) as u32;
                let raw = attr["raw"].as_object()
                    .and_then(|r| r["value"].as_u64())
                    .unwrap_or(0);
                let threshold = attr["thresh"].as_u64().unwrap_or(0) as u32;
                let flags = attr["flags"].as_object()
                    .and_then(|f| f["value"].as_u64())
                    .unwrap_or(0) as u32;
                let pre_fail = attr["flags"].as_object()
                    .and_then(|f| f["prefailure"].as_bool())
                    .unwrap_or(false);
                
                let failing = value != 0 && value <= threshold && pre_fail;
                ...
                // Extract key attrs
                match id {
                    5 => smart.reallocated_sectors = raw,
                    197 => smart.pending_sectors = raw,
                    198 => smart.uncorrectable_errors = raw,
                    _ => {}
                }
                
                smart.attributes.push(smart_attr);
            }
            ...
            let penalty = (smart.reallocated_sectors * 2 +
                          smart.pending_sectors * 2 +
                          smart.uncorrectable_errors * 5) as u32;
            smart.health_percent = std::cmp::max(0, 100 - std::cmp::min(penalty, 100));
        }
        
        // NVMe data
        if let Some(nvme_health) = data["nvme_smart_health_information_log"].as_object() {
            let nvme = NvmeData {
                temperature_c: smart.temperature_c,
                available_spare_percent: nvme_health["available_spare"].as_u64().unwrap_or(0) as u32,
                available_spare_threshold_percent: nvme_health["available_spare_threshold"].as_u64().unwrap_or(0) as u32,
                percentage_used: nvme_health["percentage_used"].as_u64().unwrap_or(0) as u32,
                data_units_written: nvme_health["data_units_written"].as_u64().unwrap_or(0),
                data_units_read: nvme_health["data_units_read"].as_u64().unwrap_or(0),
                host_write_commands: nvme_health["host_write_commands"].as_u64().unwrap_or(0),
                host_read_commands: nvme_health["host_read_commands"].as_u64().unwrap_or(0),
                media_errors: nvme_health["media_errors"].as_u64().unwrap_or(0),
                num_err_log_entries: nvme_health["num_err_log_entries"].as_u64().unwrap_or(0),
                power_on_hours: smart.power_on_hours,
                unsafe_shutdowns: nvme_health["unsafe_shutdowns"].as_u64().unwrap_or(0),
                critical_warning: nvme_health["critical_warning"].as_u64().unwrap_or(0) as u32,
            };
            
            smart.nvme = Some(nvme);
            smart.health_percent = std::cmp::max(0, 100 - nvme.percentage_used);
    ...
    fn collect_motherboard(&self) -> Result<MotherboardInfo> {
        ...
        for (path, attr) in &dmi_fields {
            if Path::new(path).exists() {
                if let Ok(content) = self.read_file_to_string(path) {
                    ...
                    match *attr {
                        "manufacturer" => mb.manufacturer = value,
                        "model" => mb.model = value,
                        "version" => mb.version = value,
                        "bios_vendor" => mb.bios_vendor = value,
                        "bios_version" => mb.bios_version = value,
                        "bios_date" => mb.bios_date = value,
                        _ => {}
                    }
                }
            }
        ...
        if let Ok(output) = Command::new("lspci")
            ...
            .output() {
            if output.status.success() && !output.stdout.is_empty() {
                let first_line = output.stdout.lines().next().unwrap_or("");
                let parts: Vec<&str> = regex::Regex::new(r#""([^"]*)""#)?
                    .captures_iter(first_line)
                    .map(|cap| cap[1].to_string())
                    .collect();
                
                if parts.len() >= 3 {
                    mb.chipset = format!("{} {}", parts[1], parts[2]).trim().to_string();
                }
            }
        }
        ...
        if Path::new(hwmon_base).exists() {
            for entry in fs::read_dir(hwmon_base)? {
                ...
                let valid_names = [
                    "nct6775", "nct6779", "nct6776", "nct6791", "nct6792",
                    "nct6793", "nct6795", "nct6796", "nct6797", "nct6798",
                    "asus_ec", "it8720f", "it8728f", "it8771e", "w83627ehf"
                ];
                ...
                for entry in fs::read_dir(&hwmon_dir)? {
                    ...
                    if let Ok(val) = temp_str.trim().parse::<f32>() {
                        ...
                        if label_lower.contains("vrm") || label_lower.contains("vcore") {
                            mb.vrm_temp_c = val;
                        } else if label_lower.contains("pch") {
                            mb.pch_temp_c = val;
                        } else if label_lower.contains("ambient") || 
                                  label_lower.contains("system") || 
                                  label_lower.contains("board") {
                            if mb.ambient_temp_c == 0.0 {
    ...
    fn collect_hwmon_fans_voltages(&self) -> Result<(Vec<FanReading>, Vec<VoltageReading>)> {
        ...
        for entry in fs::read_dir(hwmon_base)? {
            ...
            let chip_name = self.read_file_to_string(&name_path.to_string_lossy())?
                .trim()
                .to_string();
            ...
            for entry in fs::read_dir(&hwmon_dir)? {
                ...
                let re = regex::Regex::new(r"fan(\d+)_input")?;
                if let Some(idx) = re.captures(file_name) {
                    let n = &idx[1];
                    let label_path = hwmon_dir.join(format!("fan{}_label", n));
                    let label = if label_path.exists() {
                        self.read_file_to_string(&label_path.to_string_lossy())?
                            .trim()
                            .to_string()
                    ...
                    let rpm_str = self.read_file_to_string(&file.to_string_lossy())?;
                    if let Ok(rpm) = rpm_str.trim().parse::<i32>() {
                        if rpm >= 0 && rpm <= 20000 {
                            // Target RPM for controllable fans
                            let target_path = hwmon_dir.join(format!("fan{}_target", n));
                            let target = if target_path.exists() {
                                self.read_file_to_int(&target_path.to_string_lossy())
                                    .unwrap_or(0) as u32
                            } else {
                                0
                            };
            ...
            for entry in fs::read_dir(&hwmon_dir)? {
                ...
                let re = regex::Regex::new(r"in(\d+)_input")?;
                if let Some(idx) = re.captures(file_name) {
                    let n = &idx[1];
                    let label_path = hwmon_dir.join(format!("in{}_label", n));
                    let label = if label_path.exists() {
                        self.read_file_to_string(&label_path.to_string_lossy())?
                            .trim()
                            .to_string()
                    ...
                    let mv_str = self.read_file_to_string(&file.to_string_lossy())?;
                    if let Ok(mv) = mv_str.trim().parse::<f32>() {
                        let v = mv / 1000.0;
                        if v > 0.0 && v <= 25.0 {
                            let min_v = hwmon_dir.join(format!("in{}_min", n));
                            let max_v = hwmon_dir.join(format!("in{}_max", n));
                            
                            let min_val = if min_v.exists() {
                                self.read_file_to_string(&min_v.to_string_lossy())
                                    .ok()
                                    .and_then(|s| s.trim().parse::<f32>().ok())
                                    .map(|v| v / 1000.0)
                                    .unwrap_or(0.0)
                            ...
                            let max_val = if max_v.exists() {
                                self.read_file_to_string(&max_v.to_string_lossy())
                                    .ok()
                                    .and_then(|s| s.trim().parse::<f32>().ok())
                                    .map(|v| v / 1000.0)
                                    .unwrap_or(0.0)
    ...
    fn collect_batteries(&self) -> Result<Vec<BatteryInfo>> {
        ...
        for entry in fs::read_dir(ps_base)? {
            let ps = entry?.path();
            if !ps.is_dir() {
                continue;
            }
            
            let type_path = ps.join("type");
            if !type_path.exists() {
                continue;
            }
            
            let ps_type = self.read_file_to_string(&type_path.to_string_lossy())?
                .trim()
                .to_string();
            ...
            let read_int = |fname: &str| -> Result<u64> {
                let path = ps.join(fname);
                if path.exists() {
                    self.read_file_to_int(&path.to_string_lossy()).map(|v| v as u64)
                } else {
                    Ok(0)
                }
            };
            
            let read_string = |fname: &str| -> Result<String> {
                let path = ps.join(fname);
                if path.exists() {
                    self.read_file_to_string(&path.to_string_lossy())
                } else {
                    Ok(String::new())
                }
            };
            
            bat.status = read_string("status")?.trim().to_string();
            bat.technology = read_string("technology")?.trim().to_string();
            bat.cycle_count = read_int("cycle_count")?;
            bat.voltage_mv = read_int("voltage_now")? / 1000;
            
            // Energy units (preferred) or charge units
            if ps.join("energy_full_design").exists() {
                bat.capacity_design_mwh = read_int("energy_full_design")? / 1000;
                bat.capacity_full_mwh = read_int("energy_full")? / 1000;
                bat.capacity_now_mwh = read_int("energy_now")? / 1000;
                bat.charge_rate_mw = read_int("power_now")? / 1000;
            } else if ps.join("charge_full_design").exists() {
                // Convert charge (µAh) to energy (mWh) using voltage
                let v_uv = read_int("voltage_now")?.max(3_700_000);
                bat.capacity_design_mwh = read_int("charge_full_design")? * v_uv / 1_000_000_000;
                bat.capacity_full_mwh = read_int("charge_full")? * v_uv / 1_000_000_000;
                bat.capacity_now_mwh = read_int("charge_now")? * v_uv / 1_000_000_000;
                bat.charge_rate_mw = read_int("current_now")? * v_uv / 1_000_000_000;
            }
    ...
    fn collect_pcie(&self) -> Result<Vec<PcieDevice>> {
        ...
        let output_str = String::from_utf8(output.stdout)
            .context("Failed to parse lspci output")?;
        
        let mut current = HashMap::new();
        for line in output_str.lines() {
            if line.trim().is_empty() {
                if !current.is_empty() {
                    let mut d = PcieDevice::default();
                    d.slot = current.get("Slot").cloned().unwrap_or_default();
                    d.class_name = current.get("Class").cloned().unwrap_or_default();
                    d.vendor = current.get("Vendor").cloned().unwrap_or_default();
                    d.device = current.get("Device").cloned().unwrap_or_default();
                    d.subsystem = current.get("SDevice").cloned().unwrap_or_default();
                    d.driver = current.get("Driver").cloned().unwrap_or_default();
                    ...
                    if link_path.exists() {
                        ...
                        if width_path.exists() {
                            if let Ok(width_str) = self.read_file_to_string(&width_path.to_string_lossy()) {
                                if let Ok(width) = width_str.trim().parse::<u32>() {
                                    d.width = width;
                                }
                            }
                        ...
                        if gen_path.exists() {
                            if let Ok(gen_str) = self.read_file_to_string(&gen_path.to_string_lossy()) {
                                if let Some(dot_pos) = gen_str.find('.') {
                                    if let Ok(gen) = gen_str[..dot_pos].parse::<u32>() {
                                        d.gen = gen;
                                    }
                                }
                            }
                ...
                if parts.len() == 2 {
                    current.insert(parts[0].trim().to_string(), parts[1].trim().to_string());
                }
            }
        }
    ...
    fn collect_network(&mut self) -> Result<Vec<NetworkInfo>> {
        let mut nics = Vec::new();
        let net_base = "/sys/class/net";
        ...
        let now = self.get_timestamp();
        for entry in fs::read_dir(net_base)? {
            let iface = entry?.path();
            let name = iface.file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("")
                .to_string();
            ...
            // MAC
            let mac_path = iface.join("address");
            if mac_path.exists() {
                nic.mac = self.read_file_to_string(&mac_path.to_string_lossy())?
                    .trim()
                    .to_string();
            }
            
            // Speed
            let speed_path = iface.join("speed");
            if speed_path.exists() {
                let speed_str = self.read_file_to_string(&speed_path.to_string_lossy())?;
                if let Ok(speed) = speed_str.trim().parse::<u32>() {
                    nic.speed_mbps = speed;
                }
            }
            ...
            // Link state
            let operstate_path = iface.join("operstate");
            if operstate_path.exists() {
                let state = self.read_file_to_string(&operstate_path.to_string_lossy())?
                    .trim()
                    .to_string();
            ...
            // Driver
            let driver_link = iface.join("device").join("driver");
            if driver_link.exists() && driver_link.is_symlink() {
                if let Ok(driver_path) = fs::read_link(&driver_link) {
                    nic.driver = driver_path.file_name()
                        .and_then(|n| n.to_str())
                        .unwrap_or("")
                        .to_string();
                }
            ...
            // RX/TX bytes and rates
            let rx_path = iface.join("statistics").join("rx_bytes");
            let tx_path = iface.join("statistics").join("tx_bytes");
            let rx_err_path = iface.join("statistics").join("rx_errors");
            let tx_err_path = iface.join("statistics").join("tx_errors");
            
            if rx_path.exists() && tx_path.exists() {
                let rx = self.read_file_to_int(&rx_path.to_string_lossy())? as u64;
                let tx = self.read_file_to_int(&tx_path.to_string_lossy())? as u64;
                let rx_err = self.read_file_to_int(&rx_err_path.to_string_lossy()).unwrap_or(0) as u64;
                let tx_err = self.read_file_to_int(&tx_err_path.to_string_lossy()).unwrap_or(0) as u64;
                ...
                let prev = self.prev_net.get(&name);
                if let Some((prx, ptx, pt)) = prev {
                    let dt = now - pt;
                    if dt > 0.0 {
                        nic.rx_rate_mbs = (rx - prx) as f32 / 1e6 / dt as f32;
                        nic.tx_rate_mbs = (tx - ptx) as f32 / 1e6 / dt as f32;
                    }
                }
                self.prev_net.insert(name, (rx, tx, now));
            }
...
pub struct HardwareInfoCollector {
    backend: LinuxHardwareCollector,
}

impl HardwareInfoCollector {
    pub fn new() -> Self {
        Self {
            backend: LinuxHardwareCollector::new(),
        }
    }

    pub fn snapshot(&mut self) -> Result<HardwareSnapshot> {
        self.backend.collect()
    }

    pub async fn async_snapshot(&mut self) -> Result<HardwareSnapshot> {
        // In a real implementation, this would run the collection in a separate thread
...
pub struct SensorPoller {
    ...
    latest: Option<HardwareSnapshot>,
    stop: bool,
}
...
impl SensorPoller {
    pub fn new(interval_ms: u64) -> Self {
        Self {
            ...
            collector: HardwareInfoCollector::new(),
            latest: None,
            stop: false,
        }
    }
    ...
    pub async fn run(&mut self) {
        while !self.stop {
            match self.collector.snapshot() {
                ...
                Err(e) => eprintln!("Hardware sensor poll error: {}", e),
            }
